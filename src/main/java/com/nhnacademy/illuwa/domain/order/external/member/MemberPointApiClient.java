...
public interface MemberPointApiClient {
    ...
    @PostMapping(value = "/members/{memberId}/points/earn")
    MemberSavePointDto sendTotalPrice(@PathVariable Long memberId, BigDecimal totalPrice);

    // 해당 멤버의 포인트 조회
    @GetMapping(value = "/members/{memberId}/points")
    Optional<MemberPointDto> getPointByMemberId(@PathVariable Long memberId);
